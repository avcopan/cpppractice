...
class SimpleTensorContractionMapper {
  ...
  inline void prefetch(Index /*i*/) { }

  
  inline Scalar operator()(Index row) const {
  ...
  inline Index computeIndex(Index row, Index col) const {
  ...
  inline IndexPair<Index> computeIndexPair(Index row, Index col, const Index distance) const {
    const bool left = (side == Lhs);
  ...
  class BaseTensorContractionMapper : public SimpleTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, packet_size, inner_dim_contiguous>
{
 public:
  ...
  inline Packet loadPacket(Index i, Index j) const {
    // whole method makes column major assumption

    // don't need to add offsets for now (because operator handles that)
    // current code assumes packet size must be a multiple of 2
    static_assert(packet_size % 2 == 0, "you made a programming mistake");
  ...
  inline HalfPacket loadHalfPacket(Index i, Index j) const {
    // whole method makes column major assumption
...
class BaseTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, 1, inner_dim_contiguous, inner_dim_reordered, Alignment> : public SimpleTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, 1, inner_dim_contiguous>
{
 public:
  ...
  inline Packet loadHalfPacket(Index i, Index j) const {
    return loadPacket(i, j);
  }
};
...
class TensorContractionSubMapper {
 public:
  ...
  inline Packet loadPacket(Index i) const {
  ...
  inline HalfPacket loadHalfPacket(Index i) const {
    return m_base_mapper.loadHalfPacket(i + m_vert_offset, m_horiz_offset);
  }

  inline void storePacket(Index i, Packet p) const {
    m_base_mapper.storePacket(i + m_vert_offset, m_horiz_offset, p);
  }

  inline LinearMapper getLinearMapper(Index i, Index j) const {
    return LinearMapper(m_base_mapper, i + m_vert_offset, j + m_horiz_offset);
  }

  template <typename PacketT, int AlignmentType>
  inline PacketT load(Index i) const {
    static_assert((internal::is_same<PacketT, Packet>::value), "you made a programming mistake");
    static_assert((AlignmentType == Aligned || Alignment == Unaligned), "you made a programming mistake");
    return loadPacket(i);
  }
...
class TensorContractionInputMapper
 ...
 public:
  ...
  inline SubMapper getSubMapper(Index i, Index j) const {
    return SubMapper(*this, i, j);
  }

  inline VectorMapper getVectorMapper(Index i, Index j) const {
...
struct TensorContractionEvaluatorBase
{
  ...
  TensorContractionEvaluatorBase(const XprType& op, const Device& device)
    ...
    m_rightImpl(choose(Cond<static_cast<int>(Layout) == static_cast<int>(ColMajor)>(),
        ...
        m_device(device),
        m_result(NULL) {
    static_assert((static_cast<int>(TensorEvaluator<LeftArgType, Device>::Layout) ==
  ...
  inline bool evalSubExprsIfNeeded(Scalar* data) {
