...
struct IndexList : std::tuple<FirstType, OtherTypes...> {
  inline constexpr DenseIndex operator[] (const DenseIndex i) const {
    return internal::tuple_coeff<std::tuple_size<std::tuple<FirstType, OtherTypes...> >::value-1>::get(i, *this);
  }
  inline void set(const DenseIndex i, const DenseIndex value) {
...
template <typename T>
struct index_known_statically {
  inline bool operator() (DenseIndex) const{
    return false;
  }
};
...
template <typename T>
struct index_statically_eq {
  inline bool operator() (DenseIndex, DenseIndex) const{
    return false;
  }
};
