...
class TensorReshapingOp : public TensorBase<TensorReshapingOp<NewDimensions, XprType>, WriteAccessors>
{
  ...
  inline TensorReshapingOp(const XprType& expr, const NewDimensions& dims)
    ...
    inline TensorReshapingOp& operator = (const TensorReshapingOp& other)
    {
    ...
    inline TensorReshapingOp& operator = (const OtherDerived& other)
    {
...
struct TensorEvaluator<const TensorReshapingOp<NewDimensions, ArgType>, Device>
{
  ...
  inline TensorEvaluator(const XprType& op, const Device& device)
...

{
  ...
  inline CoeffReturnType& coeffRef(Index index)
  {
...
class TensorSlicingOp : public TensorBase<TensorSlicingOp<StartIndices, Sizes, XprType> >
{
  ...
  inline TensorSlicingOp(const XprType& expr, const StartIndices& indices, const Sizes& sizes)
    ...
    inline TensorSlicingOp& operator = (const OtherDerived& other)
    {
    ...
    inline TensorSlicingOp& operator = (const TensorSlicingOp& other)
    {
...
template <typename Index, typename Device> struct MemcpyTriggerForSlicing {
  MemcpyTriggerForSlicing(const Device& device) : threshold_(2 * device.numThreads()) { }
  bool operator ()(Index val) const { return val > threshold_; }
...
struct TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>, Device>
{
  ...
  inline PacketReturnType packet(Index index) const
  {
    ...
    static_assert(packetSize > 1, "you made a programming mistake");
  ...
  inline CoeffReturnType coeff(const array<Index, NumDims>& coords)
  {
 ...
 protected:
  inline Index srcCoeff(Index index) const
  {
    Index inputIndex = 0;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      for (int i = NumDims - 1; i > 0; --i) {
  ...
  : public TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>, Device>
{
  ...
  inline CoeffReturnType& coeffRef(const array<Index, NumDims>& coords)
  {
    array<Index, NumDims> inputCoords;
    for (int i = 0; i < NumDims; ++i) {
      inputCoords = coords[i] + this->m_offsets[i];
