...
class TensorBroadcastingOp : public TensorBase<TensorBroadcastingOp<Broadcast, XprType>, ReadOnlyAccessors>
{
  ...
  inline TensorBroadcastingOp(const XprType& expr, const Broadcast& broadcast)
...
struct TensorEvaluator<const TensorBroadcastingOp<Broadcast, ArgType>, Device>
{
    ...
    : m_impl(op.expression(), device)
  {
    ...
    static_assert(NumDims > 0, "you made a programming mistake");
  ...
  inline CoeffReturnType coeffColMajor(Index index) const
  {
    Index inputIndex = 0;
  ...
  inline CoeffReturnType coeffRowMajor(Index index) const
  {
    Index inputIndex = 0;
  ...
  template<int LoadMode>
  inline PacketReturnType packetColMajor(Index index) const
  {
  ...
  template<int LoadMode>
  inline PacketReturnType packetRowMajor(Index index) const
  {
    const int packetSize = internal::unpacket_traits<PacketReturnType>::size;
    static_assert(packetSize > 1, "you made a programming mistake");
    eigen_assert(index+packetSize-1 < dimensions().TotalSize());
