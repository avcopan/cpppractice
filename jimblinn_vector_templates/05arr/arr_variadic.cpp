...
int get_global_index(std::vector<int>::const_iterator& it)
{
  return 0;
}
...
inline const
int get_global_index(std::vector<int>::const_iterator& it, int first, Types ... rest)
{
  return first * (*it) + get_global_index(++it, rest...);
}
...
class Tensor {
  ...
  public:
    ...
    dtype  operator()(Index... tensor_index) const
    {
      std::vector<int>::const_iterator stride_iter = _stride.begin();
      return _array[ get_global_index(stride_iter, tensor_index...) ];
    }

    template<typename... Index>
    dtype& operator()(Index... tensor_index)
    {
      std::vector<int>::const_iterator stride_iter = _stride.begin();
      return _array[ get_global_index(stride_iter, tensor_index...) ];
    }
